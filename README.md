...
| `session_pattern` | `"((^\|/)claude( \|$)\|--sdk-url)"` | pgrep -f pattern for session processes; the default matches the `claude` CLI by name *or* the `--sdk-url` flag (Linux spawns sessions via a versioned binary not named `claude`) |
